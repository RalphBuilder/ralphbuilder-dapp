import Image from "next/image";
import Counter from "../Counter";
import Star from "@/public/images/star-two.png";

const CounterOne = () => {
  return (
    <section className="counter section pb-0 fade-wrapper">
      <div className="container">
        <div className="row">
          <div className="col-12">
            <div className="counter__inner">
              <div className="counter__single ">
                <h2 className="light-title">
                  <span className="odometer">
                    <Counter value={1} />
                  </span>
                  <span className="prefix"> K</span>
                </h2>
                <p className="primary-text">Smart Contract Created</p>
              </div>
              <div className="counter__single d-none d-xxl-block">
                <Image src={Star} alt="Image" priority />
              </div>
              <div className="counter__single ">
                <h2 className="light-title">
                  <span className="odometer"></span>
                  <Counter value={100} />
                  <span className="prefix"> %</span>
                </h2>
                <p className="primary-text">AI-Powered</p>
              </div>
              <div className="counter__single d-none d-xxl-block">
                <Image src={Star} alt="Image" priority />
              </div>
              <div className="counter__single ">
                <h2 className="light-title">
                  <span className="odometer">
                    <Counter value={1} />
                  </span>
                  <span className="prefix"> M</span>
                </h2>
                <p className="primary-text">Token in Circulation</p>
              </div>
              <div className="counter__single d-none d-xxl-block">
                <Image src={Star} alt="Image" priority />
              </div>
              <div className="counter__single ">
                <h2 className="light-title">
                  <span className="odometer">
                    <Counter value={5} />
                  </span>
                  <span className="prefix"> +</span>
                </h2>
                <p className="primary-text">Years Experience</p>
              </div>
            </div>
          </div>
        </div>
      </div>
    </section>
  );
};

export default CounterOne;
